...
def save_data(df):
    ...
    try:
        ...
        st.warning(f"Could not write to Excel: {e}. Data saved in session.")
...
def main():
    ...
    with st.sidebar:
        ...
        # Safe upload with Append/Replace modes
        upload_mode = st.radio("Upload mode", ["➕ Append (add new rows)", "🔄 Replace all (admin only)"], index=0, key="upload_mode")

        admin_pass = ""
        ADMIN_PASSWORD = "admin@hub2026"
        if upload_mode == "🔄 Replace all (admin only)":
        ...
        if uploaded:
            try:
                ...
                st.info(f"📄 **{uploaded.name}** — **{len(new_df)} rows**")

                if "➕ Append" in upload_mode:
                    existing = get_data()
                    existing_urls = set(existing["URL"].str.strip().str.lower())
                    new_rows = new_df[~new_df["URL"].str.strip().str.lower().isin(existing_urls)]
                    dupes = len(new_df) - len(new_rows)
                    if dupes > 0:
                        st.warning(f"⚠️ {dupes} duplicates skipped")
                    if len(new_rows) > 0:
                        if st.button(f"✅ Append {len(new_rows)} new records", type="primary"):
                            combined = pd.concat([existing, new_rows], ignore_index=True)
                            save_data(combined)
                            st.success(f"✅ Added {len(new_rows)} records. Total: {len(combined)}")
                            st.rerun()
                    else:
                        st.info("No new records — all URLs already exist.")
                else:
                    if admin_pass == ADMIN_PASSWORD:
                        st.warning(f"⚠️ REPLACE all {len(get_data())} records with {len(new_df)}?")
                        if st.button("🔴 Yes, Replace All", type="primary"):
                            save_data(new_df)
                            st.success(f"✅ Replaced with {len(new_df)} records")
                            st.rerun()
                    else:
                        st.error("Enter admin password for Replace mode.")
            except Exception as e:
                st.error(f"Error: {e}")
